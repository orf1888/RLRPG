...
public class SkillsListAdapter extends ListAdapter {
	...
	class SkillDialogOnCliclListener implements
		...
		@Override
		public void onClick(DialogInterface dialog, int which) {
			SkillTainingDialog improveDialog = new SkillTainingDialog(activity,
					skill.getName(),
					"How many lines of code did you wrote today?", null, null,
					"Submit", "Back", new EditText(activity));
	...
	OnClickListener rowOnClickListener = new OnClickListener() {
		...
		public void onClick(View view) {
			...
			SkillDialogOnCliclListener trainListener = new SkillDialogOnCliclListener(
					skill);

			DialogBuilder dialog = new DialogBuilder(activity, skill.getName(),
					skill.getDescr(), trainListener, returnListener, "Train",
					"Back");
			dialog.showDialog();
		}
	};
